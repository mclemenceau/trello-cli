...
def drawCards(cardList, label=False):
    if cardList:
        for card in cardList:
            labels = ""
            if card.labels and label:
                labels = " ".join("[" + lab.name + "]" for lab in card.labels)
            if len(card.name) < 70:
                print("    * {} {}".format(labels, card.name))
            else:
                print("    * {} {}...".format(labels, card.name[:68]))
...
def main():
    ...
    try:
        ...
        save_token = input(
                "Do you want to save those credentials for future "
    ...
    if not opts.members:
        ...
        for lane in the_board_lanes:
            ...
            for card in list(filter(lambda x: len(x.member_id) == 0 and
                             x.list_id == the_board_lanes[lane],
                             the_board_cards)):
        ...
        for member in the_board_members:
            if opts.members.split(",")[0] == "all" or listInString(
                    opts.members, member):
                if list(filter(lambda x: the_board_members[member] in
                        x.member_id, the_board_cards)):
                    print(member)
                    for lane in the_board_lanes:
                        card_list = []
                        for card in list(
                            filter(lambda x: the_board_members[member] in
                                   x.member_id and
                                   x.list_id == the_board_lanes[lane],
                                   the_board_cards)):
                            card_list.append(card)
